...
object ICP5 {
  def main(args: Array[String]) {
    ...
    print("stateful queries")
     ...
     stateful.show();
    //subgraph with condition
    print("subgraph with condition")

    val subgraph1 = stationGraph.find("(a)-[e]->(b)")
      .filter("e.Duration >932").show()

    val ver = stationGraph.degrees
    ver.show(5)
    println("Degree" + ver.orderBy(desc("Degree")).limit(5))

    stationGraph.vertices.write.csv("/Users/bhavy/OneDrive/Desktop/Git/BigDataProgramming/Module2/ICP5/Graphs/Vertices")

    stationGraph.edges.write.csv("/Users/bhavy/OneDrive/Desktop/Git/BigDataProgramming/Module2/ICP5/Graphs/Edges")

    val heighestdestination = stationGraph
      .edges
      .groupBy("src", "dst")
      .count()
      .orderBy(desc("count"sni))
      .limit(10)
